...
const SESSION_STORE=(()=>{try{return window.sessionStorage}catch(e){reportWarning("storage.session","Session storage is unavailable in this environment.");return null}})();
const LOCAL_STORE=(()=>{try{return window.localStorage}catch(e){reportWarning("storage.local","Local storage is unavailable in this environment.");return null}})();
const SafeStorage={
...
function saveCardHistory(obj){
  if(!SafeStorage.jsonSet(SESSION_STORE,CARD_HISTORY_KEY,obj)) reportWarning("card-history","The draw history cannot be saved in this browser.");
}
...
function randomModeDesc(){
  return randomMode==="ritual"
    ? "Druidic flow: cryptographic randomness with continuous, gentle repetition control and house balancing. There is no exclusion list: every available card always keeps a real chance."
    : "Pure draw: direct, memory-free cryptographic selection in which every available card has the same base probability.";
}
...
(function initJournalStorage(){
  ...
  if(!journalPersistOK){reportWarning("journal","The local journal is unavailable in this browser.");return;}
...
function cleanQuestion(raw){
  const q=(raw||"").trim();
  if(!q)return {original:q,suggestion:"What do I need to see more clearly in myself and in my next step?",changed:true,reason:"Instead of an empty question, the cauldron suggests an open and responsible focus."};
  let suggestion=q;
  let reason="The question is workable: it is open and returns attention to your own responsibility.";
  ...
  if(/^(visszajön|szeret|akar|fog|lesz|sikerül|megcsal|elhagy|felvesz|nyerek|kapok|összejön|megkeres|ír|hív)\b|\?$/.test(lower) && !/mit|hogyan|mire|miben|miért|merre/.test(lower)){
    suggestion="What do I need to understand about this situation, and what clear step can I responsibly take?";
    reason="Ceridwen has reframed the yes–no question as a question of self-inquiry.";
  }
  if(/mikor|hány nap|dátum|időpont|év|hónap/.test(lower)){
    suggestion="What inner and outer conditions need to come together for this process to arrive?";
    reason="Instead of asking for a date, the question now focuses on the conditions of the process.";
  }
  if(/mit gondol|mit érez|miért nem|ő akar|ő szeret|rólam|velem kapcsolatban/.test(lower)){
    suggestion="What does this relationship show me about myself, my boundaries and my next worthy step?";
    reason="Instead of examining another person’s inner world, the question returns to your own sphere of responsibility.";
  }
  if(/rontás|átok|sötét|kötés|negatív energia|valaki árt/.test(lower)){
    suggestion="How do I experience this situation as pressure or a repeating pattern, and how can I set a clearer boundary?";
    reason="The fear-based wording has been reframed as symbolic self-inquiry focused on boundaries.";
  }
...
function applyQuestionRefiner(box){
  ...
  out.innerHTML=`<strong>Refined question:</strong> ${lit(r.suggestion)}<br><span style="color:var(--mist)">${lit(r.reason)}</span>`;
}
...
function deckTone(card){
  const map={
    ust:{title:"Cauldron tone",text:"Initiatory, slow and ripening: do not seek a rushed answer; notice what raw material is becoming a new form within you."},
    istennok:{title:"Goddess layer",text:"Receptive strength, dignity and boundaries: notice what should be held gently and what should be returned clearly."},
    tajak:{title:"Avalonian landscape",text:"Read it as an inner place: where is the soul calling you — to water, garden, spring, boat or threshold?"},
    osvenyek:{title:"Path of Awen",text:"The card asks for one small walkable step. Inspiration becomes true when it becomes movement."},
    orzok:{title:"Guardian / Camelot layer",text:"Decision, discipline and vow: what is the one honourable step you will take within twenty-four hours?"},
    kristaly:{title:"Crystal layer",text:"Ground and simplify: hand on chest, feet on the floor, a glass of water, one object put in order. Bodily stability supports clarity."},
    hold:{title:"Lunar layer",text:"Read cyclically: not everything is meant to be born now. Some things grow, some become full, some wane, and some ripen in silence."},
    angyal:{title:"Angelic layer",text:"Keep it brief and clear: one sentence of message, one of protection and one of action. Too many words can dim the light."}
  };
  return map[card.h]||{title:"Deck tone",text:"Read the card in the rhythm of its own house."};
}
...
function integrationText(){
  ...
  return [a&&`Understanding: ${a}`,b&&`Release: ${b}`,c&&`24-hour step: ${c}`].filter(Boolean).join("\n");
}
...
function renderPakli(){
  ...
  document.querySelectorAll("#pakliRow .chip").forEach(b=>b.addEventListener("click",()=>{
    if(!window.ritualDone){
      ...
      if(note) note.textContent="Attunement is required first — the decks open after the cleansing sequence.";
      return;
    }
...
function guideNext(targetSelector,text){
  ...
  if(label)label.textContent=text||"The next step is waiting below.";
...
function quickTune(){
  ...
  document.querySelectorAll(".rg-text").forEach(t=>{
    t.textContent="One slow breath in: I call my attention back. One slow breath out: I release the noise. The connection is alive — the answer may serve only what is aligned with the highest good.";
  });
  ritualSync();
}
...
function ritualSync(){
  ...
  if(db){
    db.disabled=!done;
    db.textContent=done?"Draw from the cauldron":"Attunement required first";
  }
...
$("#skipAllBtn")?.addEventListener("click",()=>{
  ...
  if(!window.ritualDone){
    const note=$("#cauldronNote");
    if(note) note.textContent="Before the first spread, Ceridwen asks for a short attunement; the three buttons guide you through it in order.";
  }
});
...
document.querySelectorAll("#sitGrid .sit-btn").forEach(b=>b.addEventListener("click",()=>{
  ...
  else if(chosenSit.tool==="cleanse"){nm="The Shadow Mirror";sn="cleansing";ds="The Black Mirror reveals symbolic shadow patterns and offers a grounded cleansing practice for each one.";}
...
function renderSpreadPicker(){
  ...
  document.querySelectorAll("#spreadPicker .spread-btn").forEach(b=>b.addEventListener("click",()=>{
    if(!window.ritualDone){
      const note=$("#cauldronNote");
      if(note) note.textContent="Attunement is required first — the deck and spread can be selected after cleansing.";
      return;
    }
    currentSpread = SPREADS.find(s=>s.id===b.dataset.id);
    renderSpreadPicker(); resetTable();
    showInlineGuide("spreadNextGuide",cerEn()?"The spread is selected. Scroll down and press: Draw from the cauldron.":"A vetés kiválasztva. Görgess lejjebb, és nyomd meg: Draw from the cauldron.");
    guideNext("#drawBtn","Következő lépés lent: Draw from the cauldron ↓");
  }));
...
const ALL_CLEAR_CARD={
  id:"all-clear",h:"hold",no:"Ø",nm:"All is well",st:"The card of clear space",
  k:["order","calm","trust"],
  u:"No pattern is showing that asks for further investigation or cleansing. The space is settled and the sign is reassuring.",
  f:"Do not search for a problem where none is present now. Keep the calm and continue with gentle awareness.",
  a:"Repeated checking can create disturbance by itself. Trust is the right practice now.",
  m:"I accept that all is well now.",
  deep:"This is a closing card. When it appears, the draw ends here: no additional card, shadow or cleansing task follows it.",
  practice:"Take three calm breaths, give thanks for the clear state, then release the need to search further.",
  question:"Can I allow myself not to look for another problem now?",
  body:"easier breathing, relaxed shoulders, quieter attention",
  ritual:"touch your heart and say: all is well now",
  allClear:true
};
...
function resetTable(){
  ...
  $("#cauldronNote").textContent="The cauldron waits quietly.";
...
$("#drawBtn")?.addEventListener("click",()=>{
  ...
  try{
    if(!window.ritualDone){
      const note=$("#cauldronNote");
      if(note) note.textContent=cerEn()?"Before the first spread, Ceridwen asks for the short three-step attunement.":"Before the first spread, Ceridwen asks for a short attunement; the three buttons guide you through it in order.";
      return;
    }
    ...
    <div class="cp-body">
      ...
      <div class="cp-form">
        ...
        <div class="cp-actions">
          <button class="btn cp-ask" type="button">Pendulum: yes / no</button>
          <button class="btn ghost cp-clean" type="button">Cleanse this card</button>
        </div>
...
function bindResultAiPrompts(root=document){
  root.querySelectorAll(".result-ai-prompt").forEach(w=>{
    ...
    w.querySelector(".result-ai-copy")?.addEventListener("click",async()=>{
      ...
      const done="Copied ✓",fail="Copy failed — select the text and copy it manually.";
      if(status)status.textContent=ok?done:fail;
      if(btn){const old=btn.textContent;btn.textContent=ok?done:"Copy failed";setTimeout(()=>{btn.textContent=old;if(status)status.textContent="";},ok?1800:2600);}
    });
  });
}
  ...
  $("#reading").innerHTML = `<div class="divider" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="#c9a86a" stroke-width="1.4"><circle cx="12" cy="12" r="5"/><circle cx="12" cy="12" r="9" opacity=".4"/></svg></div>` +
   drawn.map((d,i)=>{
    if(d.darkE){
      ...
      <h5>${cerEn()?"The Priestess’ guidance":"The Priestess’s guidance"}</h5><p>${esc(darkTip(e))}</p>
    ...
    const c=d.card; const hn=houseName(c.h); const pos=spreadPos(i);
    return `<div class="reading-card">
      ...
      ${(()=>{const m=currentMoonLens(c);return `<div class="moon-lens"><div class="moon-lens__title">${cerEn()?"Moon-aligned reading":"Mai hold szerinti olvasat"} · ${esc(m.phase)}</div><p>${esc(m.text)}</p></div>`;})()}
...
function localNormalOracle(q){
  ...
  const spine=cards.map((d,i)=>{
    const c=d.card, pos=spreadPos(i), h={name:houseName(c.h)};
    const mode=localQuestionMode(q,c);
    const tone=d.rev?`itt a lap árnyéka is látszik: ${cardShadow(c)}`:`itt a lap fénye vezet: ${mode.text}`;
    ...
    return `${pos} helyén a ${c.no} · ${c.nm} (${houseName(c.h)}) áll. ${cardCore(c)} ${tone}${deeper}${practice}${question} ${dt.title}: ${dt.text} Mai hold szerinti olvasat (${ml.phase}): ${ml.text}`;
  }).join("\n\n");
...
function localDarkOracleEN(q){
  ...
  const darkText=darks.map((d,i)=>`${i+1}. ${d.darkE.cat==="kotes"?"Binding":"Shadow"}: ${darkName(d.darkE)} (${d.lvl}%). Sign: ${darkSigns(d.darkE)} Origin pattern: ${originName(d.origin)}. Release: ${darkRelease(d.darkE)}`).join("\n\n");
...
function localDarkOracle(q){
  ...
  const strongest=[...darks].sort((a,b)=>b.lvl-a.lvl)[0];
  const darkText=darks.map((d,i)=>`${i+1}. ${d.darkE.cat==="kotes"?"Binding":"Árny"}: ${d.darkE.n} (${d.lvl}%). Jele: ${d.darkE.j} Eredete: ${d.origin.n} — ${d.origin.d} Oldása: ${d.darkE.c}`).join("\n\n");
...
function readingPromptCards(){
  return drawn.map((d,i)=>{
    const pos=spreadPos(i);
    if(d.darkE){
      return `${i+1}. ${pos}: ${d.darkE.cat==="kotes"?"Binding":"Shadow"} — ${darkName(d.darkE)} (${d.lvl}%). Origin: ${originName(d.origin)}. Meaning: ${darkDesc(d.darkE)}. Release: ${darkRelease(d.darkE)}`;
    }
    const c=d.card;
    return `${i+1}. ${pos}: ${c.no} · ${cardName(c)} (${houseName(c.h)}${d.rev?", reversed":""}). Core message: ${cardCore(c)}. ${d.rev?"Shadow":"Light"}: ${d.rev?cardShadow(c):cardLight(c)}. Affirmation: ${cardAffirm(c)}`;
  }).join("\n");
}
function buildOracleAiPrompt(q,oracleText){
  ...
  return `Please analyse the following self-inquiry card reading more deeply. Do not treat it as fortune-telling and do not provide medical, legal, financial, psychological or therapeutic professional advice. Help me notice inner patterns, questions, decision points and grounded practical steps.

CONTEXT
System: Hello It’s Me · Ceridwen’s Cauldron
Spread: ${currentSpread.nm}
Question / life situation: ${q||"no separate question was entered"}
${moon?`Today’s lunar perspective: ${moon.phase} — ${moon.text}\n`:""}
CARDS
${readingPromptCards()}

THE PRIESTESS’S READING
${oracleText}
${integration?`\nMY OWN INTEGRATION\n${integration}\n`:""}
PLEASE ANALYSE
1. Summarise the central pattern in 5–7 sentences.
2. Name the deeper inner question behind it.
3. Identify possible blind spots gently.
4. Suggest three grounded next steps for the next seven days.
5. End with one concise journal question and one non-deterministic affirmation.`;
}
function setOraclePrompt(q,oracleText){
  const wrap=$("#oraclePromptWrap"), ta=$("#oraclePromptText");
  if(!wrap||!ta)return;
  ta.value=buildOracleAiPrompt(q,oracleText);
...
$("#copyOraclePromptBtn")?.addEventListener("click",async()=>{
  ...
  if(!txt)return;
  const ok=await copyTextToClipboard(txt);
  if(btn){btn.textContent=ok?"Analysis prompt copied ✓":"Copy failed";setTimeout(()=>btn.textContent="Copy analysis prompt",ok?1800:2600);}
});
$("#oracleBtn")?.addEventListener("click",()=>{
  const out=$("#oracleOut");
  out.className="oracle-out thinking";
  out.textContent="The Priestess leans over the fire and studies the cards…";
...
$("#saveBtn")?.addEventListener("click",()=>{
  ...
  journal.unshift({
    ...
    cards:drawn.map((d,i)=>d.darkE?`${currentSpread.pos[i]}: ${d.darkE.n} (${d.origin.n}, ${d.lvl}%)`:`${currentSpread.pos[i]}: ${cardName(d.card)}${d.rev?" (reversed)":""}`),
    text:oracleText
  });
  renderJournal();journalPersist();
  $("#saveBtn").textContent="Saved ✓";
  setTimeout(()=>$("#saveBtn").textContent="Save to journal",2000);
});
function renderJournal(){
  const list=$("#journalList");
  if(!journal.length){list.innerHTML=`<div class="journal-empty">The journal is still empty — in Avalon’s mist, every story begins with a blank page.</div>`;return;}
  list.innerHTML=journal.map((j,idx)=>`
    <div class="jentry">
      ...
      ${j.integration?`<details><summary>${cerEn()?"Integration":"Integration"}</summary><div class="jtext">${esc(j.integration)}</div></details>`:""}
      ${j.text?`<details><summary>${cerEn()?"The Priestess’s reading":"The Priestess’s reading"}</summary><div class="jtext">${esc(j.text)}</div></details>`:""}
      <div class="jmeta">${esc(cerEn()?"Draw ID":"Húzásazonosító")}: ${esc(j.drawId||"legacy")} · ${esc(cerEn()?"Engine":"Motor")}: ${esc(j.engineVersion||"legacy")} · ${esc(cerEn()?"Mode":"Mód")}: ${esc(j.randomMode||"legacy")}</div><div style="margin-top:12px"><button type="button" class="btn ghost jdel" data-j="${idx}" style="padding:7px 16px;font-size:12px">${cerEn()?"Delete entry":"Delete entry"}</button></div>
    </div>`).join("");
...
$("#copyJournalBtn")?.addEventListener("click",()=>{
  ...
  const txt=journal.map(j=>`=== ${j.spread} — ${journalDisplayDate(j)} ===\n${cerEn()?"Draw ID":"Húzásazonosító"}: ${j.drawId||"legacy"} | ${cerEn()?"Engine":"Motor"}: ${j.engineVersion||"legacy"} | ${cerEn()?"Mode":"Mód"}: ${j.randomMode||"legacy"}\n${j.q?(cerEn()?"Question: ":"Question: ")+j.q+"\n":""}${cerEn()?"Cards: ":"Cards: "}${(j.cards||[]).join(" · ")}\n${j.text?"\n"+j.text+"\n":""}`).join("\n");
  const ta=document.createElement("textarea");ta.value=txt;document.body.appendChild(ta);ta.select();
  try{document.execCommand("copy");}catch(e){}
  document.body.removeChild(ta);
  $("#copyJournalBtn").textContent="Copied to clipboard ✓";
  setTimeout(()=>$("#copyJournalBtn").textContent="Copy journal to clipboard",2000);
});
let clearArm=false;
$("#clearJournalBtn")?.addEventListener("click",()=>{
  if(!journal.length)return;
  if(!clearArm){clearArm=true;$("#clearJournalBtn").textContent="Are you sure? Click once more";
    setTimeout(()=>{clearArm=false;$("#clearJournalBtn").textContent="Delete journal";},3000);return;}
  clearArm=false;journal=[];renderJournal();journalPersist();
  $("#clearJournalBtn").textContent="Journal deleted ✓";
  setTimeout(()=>$("#clearJournalBtn").textContent="Delete journal",2000);
});
(function(){
  const n=document.getElementById("journalNote");
  if(n)n.textContent=journalPersistOK
    ?"The journal is stored in this browser. It remains here when you return, but it is not available on another device or after browser data is cleared. Use export for a backup."
    :"This browser does not allow persistent storage in the current environment, so the Journal section is unavailable.";
  if(!journalPersistOK){
...
function gContextMeaning(c,msg){
  ...
  if(/love|relationship|partner|family|child|friend|mother|father|marriage|separation/.test(q))
    return {label:"In the light of your question", text:`In relationships, ${cardName(c)} asks for honest presence, clear boundaries and one action that protects dignity on both sides.`};
  if(/work|money|business|project|career|decision|job|client|abundance/.test(q))
    return {label:"In the light of your question", text:`In work and practical life, ${cardName(c)} asks you to refine priorities, remove one source of noise and make the next useful step visible.`};
  if(/soul|energy|clearing|fear|anxiety|dream|path|spirit|inner|self/.test(q))
    return {label:"In the light of your question", text:`On the inner path, ${cardName(c)} points to what is ripening, what is ready to be released and what needs patient practice.`};
  return {label:"In the light", text:cardLight(c)};
}
function gCardReply(msg){
  const c=gPickCard();
  const cm=gContextMeaning(c,msg);
  const aboutOther=/\b(they|them|their|he|she|his|her|about him|about her)\b/i.test(msg||"");
  const boundary=aboutOther
    ? "\n\nCeridwen’s note: this does not diagnose another person’s inner state or present it as fact. Read the card as guidance about your own attention, boundaries and choices."
    : "";
  return `A card rises from the cauldron:\n\n${c.no} · ${cardName(c)} — ${cardSubtitle(c)}\nHouse: ${houseName(c.h)}\nKeywords: ${cardKeys(c).join(", ")}\n\nMeaning:\n${cardCore(c)}\n\n${cm.label}:\n${cm.text}\n\nDeeper teaching:\n${cardDeep(c)}\n\nGrounded practice:\n${cardPractice(c)}\n\nQuestion for reflection:\n${cardQuestion(c)}\n\nShadow expression:\n${cardShadow(c)}\n\nAffirmation:\n“${cardAffirm(c)}”${boundary}`;
}
function gChatSend(){
...
{id:"ismeretlen",n:"Unknown Erő",t:["sz","ta","la","va"],
...
{id:"lelekszerzodes",cat:"kotes",n:"Lélekszerződés-Binding",t:["sz"],
...
{id:"fogadalom",cat:"kotes",n:"Régi Fogadalom Bindinge",t:["sz","va"],
...
{id:"szerelmi",cat:"kotes",n:"Szerelmi Binding",t:["sz"],
...
{id:"koldok",cat:"kotes",n:"Családi Köldökzsinór-Binding",t:["sz"],
...
{id:"fuggosegi",cat:"kotes",n:"Függőségi Binding",t:["sz","ta"],
...
{id:"helykotes",cat:"kotes",n:"Hely-Binding",t:["sz","la"],
...
{id:"fatyol",cat:"kotes",n:"Fátyol-Binding",t:["sz"],
...
{id:"ismeretlenkotes",cat:"kotes",n:"Unknown Binding",t:["sz","ta","la","va"],
 d:"Binding, amelynek forrása a Tükör előtt is rejtve marad — csak a húzása érződik.",
  ...
  setTimeout(()=>{
    ...
    gq("#dkResults").innerHTML=
      ...
      <div class="dk-energy">
        ...
        <div class="lvl">Shadow intensity: ${lvl}%</div>
        <div class="dk-bar"><i style="width:${lvl}%"></i></div>
        <p>${esc(e.d)}</p>
        <h5>Signs</h5><p>${esc(e.j)}</p>
        <h5>Origin</h5><p><strong style="color:var(--blossom);font-weight:600">${esc(o.n)}.</strong> ${esc(o.d)} ${esc(o.fix)}</p>
        <h5>Cleansing practice</h5><p>${esc(e.c)}</p>
        <h5>The Priestess’s guidance</h5><p>${esc(e.p)}</p>
        ${darkIncantationPanel(e,`scan-${e.id}`)}
      </div>`).join("")+
      `<div class="controls" style="margin-top:6px"><button type="button" class="btn" id="dkClean">Perform the cleansing</button></div>`;    gq("#dkClean")?.addEventListener("click",()=>{
      ...
      setTimeout(()=>{
        const bl=gq("#dkBless");
        bl.innerHTML=`<h4>The space is clear</h4>
          <p>The shadows have been released and the light called back. ${name?("„"+esc(name)+"” fölött "):""}mostantól a te figyelmed a legerősebb őrző: a rítust ismételd, valahányszor a jelek visszatérnek — a tisztaság nem állapot, hanem gyakorlat.</p>
          <p style="margin-top:10px;font-family:var(--f-serif);font-style:italic;color:var(--blossom)">„May what belongs to the light remain.”</p>`;
        bl.classList.add("on");
...
function dkRenderFilters(){
  ...
  const base=[["all",cerEn()?"All 33":"Mind a 33"],["cat:arny",cerEn()?"Shadows":"Árnyak"],["cat:kotes",cerEn()?"Bindings":"Bindingek"]];
...
function dkModal(e,origin,lvl){
  const en=cerEn();
  const cat=en?(e.cat==="kotes"?"Binding":"Shadow"):(e.cat==="kotes"?"Binding":"Árny");
  $("#scroll").innerHTML=`
    <button type="button" class="close" aria-label="${en?'Close':'Bezárás'}">×</button>
    ...
    <section><h5>${en?'The Priestess’ guidance':'The Priestess’s guidance'}</h5><p>${esc(darkTip(e))}</p></section>`;
  const veil=$("#veil");
  veil.classList.add("on");
  veil.setAttribute("aria-hidden","false");
  document.body.classList.add("modal-open");
...
const ORIGINS_RAW=[
 ...
 {w:.12,n:"Unknown eredet",d:"A Tükör nem mutatja meg, honnan érkezett — és ez így is rendben van.",fix:"Használd az univerzális kötésoldót és a négy elem Nagy Tisztítását: az eredet ismerete nélkül is teljes az oldás, ha a szándék tiszta."}
];
...
window.dtSel="en";
const DTT={en:"Energy / situation",sz:"Person",ta:"Object / device",la:"Place / home",va:"Business"};
if(gq("#dtType")){
...
gq("#pendulumSave")?.addEventListener("click",()=>{
  ...
  journal.unshift({
    ...
    date:new Date().toLocaleString(cerEn()?"en-GB":"hu-HU",{dateStyle:"long",timeStyle:"short"}),
    spread:cerEn()?"Pendulum session — Ceridwen’s pendulum":"Pendulum session — Ceridwen’s pendulum",
    q:gq("#pendulumName").value.trim(),
    cards:Inga.programs.map(p=>`${darkName(p.e)} (${originName(p.o)}, ${p.lvl}%) — released`),
    text:oracleText
  });
  renderJournal();journalPersist();
  gq("#pendulumSave").textContent="Saved ✓";
  setTimeout(()=>gq("#pendulumSave").textContent="Save to journal",2000);
});
...
const SRT_4B=[
  'Child','Woman','Man','Mother','Father','Husband','Wife','Angel','Family','Other group','Spiritual energy','Discarnate being','Separated being','Seraph','Cherub','Aunt','Uncle','Thought form','Glorified being','Guide','Doubter','Higher Self'
];
const SRT_SOURCE=['The purest source of love','Radiance','Spirit','Source','GOD','I AM','Guiding consciousness','Holy Spirit consciousness','Universal consciousness','Other / uncertain source'];
const SRT_SHADOW=()=>['None — all is well',...DARK.map(e=>darkName(e))];
const SRT_ENTRY=[
 {id:'communicator',chart:'4B',type:'custom',title:'Who am I communicating with?',options:()=>SRT_4B,pass:v=>v==='Higher Self',passLabel:'Higher Self'},
 {id:'source',chart:'3',type:'custom',title:'Where is the answer coming from?',options:()=>SRT_SOURCE,pass:v=>v==='The purest source of love',passLabel:'The purest source of love'},
 {id:'committee',chart:'4B',type:'number',title:'How many members are in my Higher Self committee?',min:1,max:10,pass:v=>v>=3&&v<=5,passLabel:'3–5'},
 {id:'lock',type:'cleanOnly',title:'Close the pendulum to conscious influence',cleanText:'I ask that the pendulum and the answer process be closed to conscious mental influence, so that only a clear and verified response remains.'},
 {id:'commitment',type:'yesno',title:'Is the Higher Self committee committed to answering from the purest source of love?',want:true},
 {id:'permission',type:'yesno',title:'Do I have permission to connect with the Higher Self?',want:true},
 {id:'ego',chart:'4B',type:'number',title:'What is my ego level?',min:0,max:10,pass:v=>v===1,passLabel:'1'},
 {id:'grounding',chart:'4B',type:'number',title:'What is my grounding level?',min:0,max:10,pass:v=>v===10,passLabel:'10'},
 {id:'selfPrograms',type:'yesno',title:'Am I running programs?',want:false},
 {id:'helperPrograms',type:'yesno',title:'Are my helpers running programs?',want:false},
 {id:'harmony',type:'yesno',title:'Are body, soul and spirit in harmony?',want:true},
 {id:'souls',chart:'4B',type:'number',title:'How many souls are within me?',min:0,max:10,pass:v=>v===1,passLabel:'1'},
 {id:'shadowSelf',type:'shadow',title:'Is there any curse, harmful influence or dark force affecting me?',target:'self'},
 {id:'gatewaySelf',type:'gateway',title:'Is there a gateway connected with the area just examined?'},
 {id:'shadowAround',type:'shadow',title:'Is there any curse, harmful influence or dark force around me?',target:'surroundings'},
 {id:'gatewayAround',type:'gateway',title:'Is there a gateway connected with the area just examined?'},
 {id:'shadowWork',type:'shadow',title:'Is there any curse, harmful influence or dark force affecting my workplace or business?',target:'workplace / business'},
 {id:'gatewayWork',type:'gateway',title:'Is there a gateway connected with the area just examined?'},
 {id:'shadowObjects',type:'shadow',title:'Is there any curse, harmful influence or dark force attached to property, family property, an object or device?',target:'objects / devices'},
 {id:'gatewayObjects',type:'gateway',title:'Is there a gateway connected with the area just examined?'},
 {id:'shadowFamily',type:'shadow',title:'Is there any curse, harmful influence or dark force affecting my family or family members?',target:'family / family members'},
 {id:'gatewayFamily',type:'gateway',title:'Is there a gateway connected with the area just examined?'},
 {id:'trueHigherSelf',type:'yesno',title:'Am I connected with my true Higher Self / Spiritual Committee?',want:true},
 {id:'channelClear',type:'yesno',title:'Is the communication channel clear?',want:true},
 {id:'workPermission',type:'yesno',title:'Do I have permission and authorization to work on this matter now?',want:true},
 {id:'highestGood',type:'yesno',title:'Does this work serve the highest good of everyone involved?',want:true},
 {id:'safe',type:'yesno',title:'Is it allowed, possible and safe to perform this research and clearing?',want:true},
 {id:'protection',type:'cleanOnly',title:'Request complete protection',cleanText:'I ask for complete and clear protection around myself, every entered participant and the entire working space.'},
 {id:'startTogether',type:'yesno',title:'May we begin the work together?',want:true},
 {id:'correctAnswers',type:'yesno',title:'Will I receive correct answers?',want:true},
 {id:'incorrectAnswers',type:'yesno',title:'Will I receive incorrect answers?',want:false}
];
const SRT_EXIT=[
 {id:'more',type:'yesno',title:'Is there any other program connected with this topic that still needs clearing?',want:false},
 {id:'finished',type:'yesno',title:'Is today’s work complete?',want:true},
 {id:'complete100',type:'yesno',title:'Has the clearing been completed 100% on every level and through every time layer of the soul?',want:true},
 {id:'closePermission',type:'yesno',title:'May I close the channel now?',want:true}
];
...
gq("#qaClean")?.addEventListener("click",()=>{
  ...
  gq("#qaStatus").textContent="The pendulum is cleansing… keep your attention on the light.";
  (function f(now){
    ...
    if(t<dur)requestAnimationFrame(f);
      ...
      gq("#qaStatus").textContent="The cleansing is complete. “May what belongs to the light remain.”";}
  })(performance.now());
});
...
function renderDailyVisitorCard(){
  ...
  const c=DECK[idx]; const house={name:houseName(c.h)};
...
/* ===== INNER COMPASS ADVENTURE — child-friendly reflection mode ===== */
(()=>{
 ...
 const dirs=[
  {name:'Courage',icon:'🦁',angle:0,msg:'A tiny brave step may help. You do not have to fight a dragon — one mini step is enough.',action:'Say out loud the first small thing you can try.'},
  {name:'Calm',icon:'🌿',angle:60,msg:'Your brain may be running in turbo mode. Slow down first, then decide.',action:'Breathe in for 4 counts and out for 5 — three times.'},
  {name:'Help',icon:'🤝',angle:120,msg:'Even superheroes have a team. Asking for help is not cheating; it is a smart strategy.',action:'Choose a trusted grown-up or friend and tell them in one sentence what is bothering you.'},
  {name:'Curiosity',icon:'🔎',angle:180,msg:'Maybe the question is not “What is wrong with me?” but “What is this feeling trying to tell me?”',action:'Write three things: what you know, what you are guessing, and what you want to ask.'},
  {name:'Kindness',icon:'💛',angle:240,msg:'Talk to yourself like you would talk to your best friend. The grumpy brain-goblin is not always right.',action:'Say one kind and true sentence to yourself.'},
  {name:'Pause',icon:'⏸️',angle:300,msg:'Sometimes the smartest move is a short pause. Your brain is not a microwave — not everything must be solved in 30 seconds.',action:'Drink water, move for two minutes, then look at the question again.'}
 ];
 let busy=false;
 ...
 $k('#kidsFeelings')?.addEventListener('click',e=>{const b=e.target.closest('button[data-feeling]');if(!b)return;document.querySelectorAll('#kidsFeelings button').forEach(x=>x.classList.toggle('on',x===b));show($k('#kidsCompassCard'));});
 $k('#kidsAsk')?.addEventListener('click',()=>{if(busy)return;busy=true;const b=$k('#kidsAsk'),status=$k('#kidsStatus'),needle=$k('#kidsNeedle');b.disabled=true;status.textContent='The compass is stretching… one moment!';const i=rnd(dirs.length),d=dirs[i],turns=720+d.angle;needle.style.transform=`translate(-50%,-100%) rotate(${turns}deg)`;window.Haptics?.play?.('start');setTimeout(()=>{status.textContent=`Direction: ${d.name}`;const q=$k('#kidsQuestion').value.trim();const result=$k('#kidsResult');result.innerHTML=`<div class="kids-result__icon">${d.icon}</div><div class="kids-result__tag">${d.name}</div><h3>${d.name} mode activated!</h3><p>${d.msg}</p>${q?`<p><strong>Your question:</strong> ${esc(q)}</p>`:''}<div class="kids-result__action">🎯 <strong>Mini mission:</strong> ${d.action}</div><p class="small-note">This is a playful idea, not a required answer. You decide whether it fits. If the question is scary, painful or about your safety, talk to a trusted grown-up.</p><div class="kids-result__buttons"><button type="button" class="btn kids-again">New direction</button><button type="button" class="btn ghost kids-reset">New question</button></div>`;result.hidden=false;result.querySelector('.kids-again').addEventListener('click',()=>{result.hidden=true;b.click()});result.querySelector('.kids-reset').addEventListener('click',()=>{$k('#kidsQuestion').value='';result.hidden=true;$k('#kidsFeelingCard').hidden=true;$k('#kidsCompassCard').hidden=true;document.querySelectorAll('#kidsFeelings button').forEach(x=>x.classList.remove('on'));needle.style.transform='translate(-50%,-100%) rotate(0deg)';$k('#kidsStartCard').scrollIntoView({behavior:'smooth',block:'start'})});b.disabled=false;busy=false;window.Haptics?.play?.('result');result.scrollIntoView({behavior:'smooth',block:'start'});},1250);});
})();
