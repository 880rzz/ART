...
const root=path.resolve(import.meta.dirname,'..'), cssPath=path.join(root,'assets/css/record-editorial-system.css'), releasePath=path.join(root,'data/design-release.json'), runtimePath=path.join(root,'assets/js/responsive-header-system.js'), auditPath=path.join(root,'tests/audit-record-editorial-stage75.mjs'), bumpPath=path.join(root,'scripts/bump-editorial-release-cache.mjs');
...
let audit=await readFile(auditPath,'utf8');audit=audit.replace('Stage 75/76/77 permanent regression gate','Stage 75/76/77/78 permanent regression gate');
...
if(!audit.includes('stage78Exhibitions'))audit=audit.replace(bookLine,bookLine+stage78Line);audit=audit.replace('Stage 75/76/77 record editorial audit passed for exhibition and book records in EN/HU/DE-AT, including four exhibition and three book signatures.','Stage 75/76/77/78 record editorial audit passed for exhibition and book records in EN/HU/DE-AT, including landmark signatures, all remaining exhibition families and three book signatures.');await writeFile(auditPath,audit);
...
const release=JSON.parse(await readFile(releasePath,'utf8'));release.release=newRelease;release.note='Stage 78 exhibition families: all remaining exhibition records gain deliberate portrait-cycle, New York, intimate-story, contemporary-identity, AI-frame or group-dossier pacing across EN/HU/DE-AT while preserving two-blue/gold authority, schema and routes.';release.assetDigest=hash.digest('hex').slice(0,16);await writeFile(releasePath,JSON.stringify(release,null,2)+'\n');console.log(`Stage 78 exhibition-family signatures applied; release ${newRelease}; digest ${release.assetDigest}.`);
