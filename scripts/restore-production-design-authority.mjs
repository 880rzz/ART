...
for(const name of fs.readdirSync(bundlesDir).filter(name=>/^art-[a-f0-9]{16}\.css$/.test(name))){const bundled=fs.readFileSync(path.join(bundlesDir,name),'utf8');for(const marker of ['ART-DESIGN-CONSTITUTION-MERGED:START','ART-SCREENSHOT-OPTICAL-AUTHORITY-MERGED:START','ART-MOBILE-HOME-CTA-AUTHORITY-MERGED:START','ART-VISUAL-PERFECTION-AUTHORITY-MERGED:START','ART-FINAL-GEOMETRY-CLOSURE-MERGED:START']) if(!bundled.includes(marker)) throw new Error(`${marker} missing from ${name}.`);}
