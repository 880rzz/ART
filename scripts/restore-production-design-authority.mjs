...
if (fs.existsSync(bundlesDir)) {
  for (const name of fs.readdirSync(bundlesDir)) {
    ...
    fs.writeFileSync(path.join(bundlesDir, name), sourceCss, 'utf8');
    bundles += 1;
  }
}
...
console.log(`ART production design authority restored from source: ${bundles} bundle(s), ${htmlChecked} HTML files checked, ${inlineRemoved} artifact-only style block(s) removed.`);
