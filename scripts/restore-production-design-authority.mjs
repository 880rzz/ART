...
function compileMuseumAuthority(css){
  ...
  const panel=design.palette?.panel||'#3A435D';
  let out=css;
  ...
  const pageCanvas=`min(calc(100% - ${d.gutterPx*2}px),${design.pageMaxPx}px)`;
...
/* One museum coordinate system: viewport -> page canvas -> structured record -> reading measure.
   Narrow prose is never allowed to shrink or recenter the outer page canvas. */
@media(min-width:901px){
  ...
  body.apple-archive main>section:not([data-layout="centered"])>.wrap.narrow{
    width:${pageCanvas}!important;max-width:${design.pageMaxPx}px!important;margin-left:auto!important;margin-right:auto!important;
  }
  ...
  body.apple-archive main>section:not([data-layout="centered"])>.wrap.narrow>:is(.label,.eyebrow,.kicker,h1,h2,h3,p,.lead,.description,.section-description){
  ...
  /* Community timelines and other editorial records use the structured width but stay on the page axis. */
  body.apple-archive main>section.wrap.narrow>:is(.timeline,.linklist,.facts,.curatorial-periods,.curatorial-section){
    width:min(100%,${d.curatorsStructuredMaxPx}px)!important;max-width:${d.curatorsStructuredMaxPx}px!important;margin-left:0!important;margin-right:auto!important;
  }
  body.apple-archive main>section.wrap.narrow :is(.timeline,.curatorial-periods,.curatorial-section)>:is(.label,.eyebrow,.kicker,h2,h3,p,.lead,ul,blockquote),
  body.apple-archive main>section.wrap.narrow .timeline :is(.t-item,.curatorial-section)>:is(.label,.eyebrow,.kicker,h2,h3,p,.lead,ul,blockquote){margin-left:0!important;margin-right:auto!important;text-align:left!important;}

  /* Press: common page canvas; record area is wide but left anchored. */
  ...
  /* Writing: outer page canvas is shared; list/facts are 1180px structured records, prose remains 860px. */
  ...
  /* Source hubs: shared page axis; evidence grid structured and left anchored. */
  ...
  /* Curators: periods are structured records on the shared page axis, never centered islands. */
  ...
  body.apple-archive[data-archive-page="curators"] main .curatorial-section{width:min(100%,${d.curatorsStructuredMaxPx}px)!important;max-width:${d.curatorsStructuredMaxPx}px!important;margin-left:0!important;margin-right:auto!important;text-align:left!important;}
  body.apple-archive[data-archive-page="curators"] main .curatorial-section>:is(.label,.eyebrow,.kicker,h2,h3,p,.lead,ul,blockquote){margin-left:0!important;margin-right:auto!important;text-align:left!important;max-width:${d.proseMaxPx}px!important;}
}
...
@media(max-width:900px){
  ...
  body.apple-archive[data-archive-page="curators"] main :is(.curatorial-periods,.curatorial-section){width:100%!important;max-width:none!important;margin-left:0!important;margin-right:0!important;}
  body.apple-archive[data-archive-page="curators"] main .curatorial-section>:is(.label,.eyebrow,.kicker,h2,h3,p,.lead,ul,blockquote){margin-left:0!important;margin-right:auto!important;text-align:left!important;}
}
...
if (fs.existsSync(bundlesDir)) for (const name of fs.readdirSync(bundlesDir)) {
  ...
  if(newName!==name){bundleRenames.set(`/assets/css/bundles/${name}`,`/assets/css/bundles/${newName}`);fs.rmSync(oldPath,{force:true});}
  bundles += 1;if(compiled.changed) compiledBundles += 1;
}
...
function walk(dir){
  for(const entry of fs.readdirSync(dir,{withFileTypes:true})){
    ...
    if(entry.isDirectory()) walk(full);
      ...
      htmlChecked += 1;const before=fs.readFileSync(full,'utf8');let after=before.replace(/\s*<style\s+data-exhibition-axis-contract=["']v1["']>[\s\S]*?<\/style>\s*/gi,'\n');
      ...
      for(const [oldHref,newHref] of bundleRenames){if(after.includes(oldHref)){after=after.split(oldHref).join(newHref);bundleRefsUpdated+=1;}}
      const isFullDocument=/<html\b/i.test(after)&&/<head\b/i.test(after)&&/<\/head>/i.test(after);
      if(isFullDocument){fullDocuments+=1;after=after.replace(/\s*<style\s+data-art-runtime-typography-closure=["'][^"']+["']>[\s\S]*?<\/style>\s*/gi,'');}
...
const protectedFiles={
  ...
  'professional-llm-mirror.json':['Q138482177','hipstudioFounderAuthority',"Bánhalmi Norbert and Speier Vikó are HIPStudio's main professional photographer partners",'Speier Vikó is the photography-services contact','1111 Budapest, Lágymányosi utca 15.','shared address or Google Business Profile presence does not merge the entities']
};
for(const [rel,tokens] of Object.entries(protectedFiles)){
  const file=path.join(siteRoot,rel);if(!fs.existsSync(file))throw new Error(`ART protected authority file missing from artifact: ${rel}`);
  const text=fs.readFileSync(file,'utf8');for(const token of tokens)if(!text.includes(token))throw new Error(`${rel}: protected HIPStudio authority token missing after artifact hardening: ${token}`);
}
...
for(const newHref of bundleRenames.values()){
  const full=path.join(siteRoot,newHref.replace(/^\//,''));if(!fs.existsSync(full))throw new Error(`ART re-hashed design bundle missing: ${newHref}`);
  const css=fs.readFileSync(full,'utf8');
  for(const retired of (design.palette?.retiredPanelAliases||[]))if(css.toLowerCase().includes(retired.toLowerCase()))throw new Error(`ART retired panel surface remains in ${newHref}: ${retired}`);
  for(const required of ['ART-MACHINE-DESIGN-AUTHORITY',design.palette.panel,`max-width:${design.pageMaxPx}px!important`,`max-width:${design.desktop.proseMaxPx}px!important`,`.linklist>li{width:100%!important;max-width:none!important;}`,'.archive-source-hub{width:min(100%',`.curatorial-section{width:min(100%,${design.desktop.curatorsStructuredMaxPx}px)!important`])if(!css.includes(required))throw new Error(`ART machine design contract missing from ${newHref}: ${required}`);
}
console.log(`ART production design compiled from ${design.version}: ${compiledBundles}/${bundles} bundle(s) recompiled and content-hashed; ${htmlChecked} HTML files checked, ${bundleRefsUpdated} bundle reference update(s), ${fullDocuments} full documents, ${inlineRemoved} artifact HTML file(s) normalized, ${deadExhibitionCtasRemoved} dead exhibition CTA remnant(s) removed. One page axis, left-anchored structured records, narrow prose and canonical ${design.palette.panel} panel are enforced on the rendered component.`);
