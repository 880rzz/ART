...
for (const name of files) {
  ...
  if (!html.includes('>Archívumi összefoglaló<')) failures.push(`${name}: hiányzó Archívumi összefoglaló`);
  ...
  if (!html.includes('<strong>Helyszín:</strong>')) failures.push(`${name}: hiányzó helyszín`);
