...
const story='https://blog.banhalmi.art/blog/categories/noi-tortenetek';
...
if(errors.length){console.error(`Stage50 critical ecosystem audit failed with ${errors.length} issue(s):`);errors.forEach(e=>console.error(` - ${e}`));process.exit(1);}console.log(`Stage50 critical ecosystem audit passed: ${html.length} HTML, ${indexable} indexable, ${redirects} redirect surfaces inventoried; canonical redirect behavior is enforced by the dedicated internal/legacy redirect audits; JSON/JSON-LD, EN/HU/DE entity and book-blog ecosystem bridges are locked.`);
