...
// Permanent Stage 42 contract: all ART surfaces use the canonical blue scale, and the homepage motion layer is desktop-hover only while coarse/mobile devices remain video-free.
...
const js=fs.readFileSync(path.join(root,'assets/js/hero-hover-video.js'),'utf8');for(const t of ['prefers-reduced-motion','window.innerWidth<=900','mouseenter','mouseleave','pointerenter','pointerleave','canplay'])if(!js.includes(t))errors.push('hero JS missing '+t);
for(const t of ['@media (hover:none),(pointer:coarse)','display:none!important'])if(!base.includes(t))errors.push('mobile hero-video CSS contract missing '+t);
...
if(errors.length){console.error(errors.join('\n'));process.exit(1)}console.log('Stage 42 Apple surface + desktop hover-video audit passed.');
