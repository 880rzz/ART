...
if(typeof release!=='string'||!/^\d{8}-.+-v\d+$/.test(release))errors.push('invalid active release '+release);
...
for(const [name,fg] of [['primary','#F5F5F7'],['secondary','#AFC4D9'],['gold','#DCC56B']])for(const bg of ['#202530','#29303F','#484F60']){const r=cr(fg,bg);if(r<4.5)errors.push(name+' '+r.toFixed(2)+' on '+bg);else console.log(name+' / '+bg+': '+r.toFixed(2)+':1')}
