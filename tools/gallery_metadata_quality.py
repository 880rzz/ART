...
def update_html_images(source: str, copy: dict, total: int) -> str:
    def replace(match: re.Match) -> str:
        ...
        if needs_replacement(alt):
            tag = set_attr(tag, 'alt', copy['alt'].format(index=index, total=total))
        ...
        for node in walk_nodes(data):
            ...
            for position, image in enumerate(media, start=1):
                if not isinstance(image, dict):
                    continue
                if needs_replacement(str(image.get('name', ''))):
                    image['name'] = copy['name'].format(index=position, total=total)
                if needs_replacement(str(image.get('caption', ''))):
                    image['caption'] = copy['alt'].format(index=position, total=total)
