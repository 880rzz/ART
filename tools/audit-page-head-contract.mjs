...
function routeFor(rel){if(rel==='index.html')return'/';return'/'+rel;}
...
function inspect(rel,file){const h=fs.readFileSync(file,'utf8');if(isRedirect(h)||!/<main\b/i.test(h))return;const visible=(h.match(/<main\b[\s\S]*?<\/main>/i)?.[0]||'').replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim();if(visible.length<160)return;pages++;
 ...
 const desc=meta(h,'description');if(!desc)failures.push(`${rel}: meta description missing`);
...
walk('.');if(pages<40)failures.push(`real ART head-contract coverage unexpectedly low: ${pages}`);if(failures.length){console.error('ART page-head contract FAILED:\n'+failures.map(x=>' - '+x).join('\n'));process.exit(1)}console.log(`ART page-head contract passed across ${pages} real content pages.`);
