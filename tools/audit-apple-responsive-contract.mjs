...
if (b >= 0) {
  ...
  else if (css.slice(markerClose + 2).trim()) failures.push('Apple responsive compatibility template must remain the final committed CSS block; rules found after END marker');
}
const contract = a >= 0 && b > a ? css.slice(a,b) : '';

if (!Number.isFinite(Number(design.pageMaxPx)) || Number(design.pageMaxPx) < 960 || Number(design.pageMaxPx) > 1800) failures.push(`invalid machine design pageMaxPx: ${design.pageMaxPx}`);
if (!design.desktop || !Number.isFinite(Number(design.desktop.structuredMaxPx))) failures.push('machine design authority missing desktop structuredMaxPx');
if (design.principles?.runtimeCssInjection !== false) failures.push('machine design authority must prohibit runtime CSS injection');
...
]) if (!contract.includes(needle)) failures.push(`compatibility contract missing: ${needle}`);

const compiler = fs.readFileSync('scripts/restore-production-design-authority.mjs','utf8');
for (const token of ['data/design-authority.json','ART-MACHINE-DESIGN-AUTHORITY','createHash','bundleRenames']) if (!compiler.includes(token)) failures.push(`production design compiler contract missing: ${token}`);
if (!compiler.includes('pageMaxPx')) failures.push('production design compiler does not consume machine pageMaxPx');
...
console.log(`ART responsive authority contract passed: ${realPages.length} archive HTML files; committed CSS is the compatibility template and ${design.version} is the machine production design authority (${design.pageMaxPx}px page canvas).`);
