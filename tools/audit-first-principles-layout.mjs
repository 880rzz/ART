...
// Canonical viewport matrix + a wide desktop guard. The reported regressions
// included iPad/tablet layouts and desktop pages that were visually trapped in
// a phone-width centred column despite having much more canvas available.
...
for(const width of widths){
 ...
 for(const pathname of pages){
  ...
  const issues=await page.evaluate(()=>{
   ...
   for(const el of document.querySelectorAll('main section')){if(!visible(el)||el.matches('.hero,.hero-section,.statement,.archive-statement,.editorial-statement,.immersive,.cta-band,[data-layout="immersive"]')||el.closest('.gallery'))continue;const r=el.getBoundingClientRect(),text=(el.textContent||'').replace(/\s+/g,' ').trim(),media=el.querySelectorAll('img,video,figure,.gallery,.collage').length;if(w>=1024&&r.height>1500&&text.length<420&&media<2)out.push(`${name(el)} sparse section ${r.height.toFixed(0)}px / ${text.length} chars`);const s=getComputedStyle(el);const compact=text.length<760&&media===0;const limit=compact?(w>=1280?88:104):112;if(w>=1024&&(px(s.paddingTop)>limit||px(s.paddingBottom)>limit)&&!el.matches('.presence-context'))out.push(`${name(el)} excessive section padding ${s.paddingTop}/${s.paddingBottom} for ${compact?'compact':'content'} section`)}

   for(const h of document.querySelectorAll('main h1,main h2,main h3,header h1')){if(!visible(h))continue;const fs=px(getComputedStyle(h).fontSize);let min=0,max=999;if(h.matches('h1')){min=w<=430?36:w<=768?40:44;max=w<=430?52:w<=768?62:84}else if(h.matches('h2')){min=w<=430?27:w<=768?29:32;max=w<=430?42:w<=768?48:56}else{min=18;max=32}if(fs<min||fs>max)out.push(`${name(h)} type scale ${fs.toFixed(1)}px outside ${min}-${max}px`)}

   for(const h of document.querySelectorAll('main h1,main h2,main h3')){if(!visible(h))continue;const n=h.nextElementSibling;if(!n||!visible(n)||!n.matches('p,.lead,.description,.section-description,.hero-description,.presence-copy'))continue;const gap=n.getBoundingClientRect().top-h.getBoundingClientRect().bottom;const min=10,max=h.matches('h1')?46:h.matches('h2')?32:28;if(gap<min||gap>max)out.push(`${name(h)} → ${name(n)} gap ${gap.toFixed(0)}px outside ${min}-${max}px`)}

   // A section intro has one semantic alignment. Only explicitly curatorial,
   // statement or CTA components may centre their typography.
   for(const intro of document.querySelectorAll('main .section-head,main .section-intro,main .curatorial-periods__intro,main .press-section__intro,main section.wrap.narrow')){if(!visible(intro))continue;const nodes=[...intro.children].filter(el=>visible(el)&&el.matches('h1,h2,h3,p,.lead,.label,.presence-kicker,.period-no')&&!(el.matches('ul,ol')));if(nodes.length<2)continue;const lefts=nodes.map(el=>el.getBoundingClientRect().left),spread=Math.max(...lefts)-Math.min(...lefts);if(!centredContext(intro)&&spread>5)out.push(`${name(intro)} optical-axis drift ${spread.toFixed(1)}px`);if(!centredContext(intro)){for(const el of nodes){const a=getComputedStyle(el).textAlign;if(!['left','start'].includes(a))out.push(`${name(intro)} unexpected ${a} alignment on ${name(el)}`)}}}
   ...
   // Wide desktop: structured archive content should use the canvas while the
   // prose inside it retains a readable measure. This directly guards the
   // writing/memberships/sources/press screenshots supplied by the owner.
   if(w>=1280){const structuredSelectors=['.archive-source-hub','.press-redesign','.press-list','.press-records','.facts','.archive-grid','.books-grid','.curators-grid'];for(const sel of structuredSelectors){for(const block of document.querySelectorAll(`main ${sel}`)){if(!visible(block))continue;const r=block.getBoundingClientRect(),required=Math.min(w*.72,1380);if(r.width<required)out.push(`${name(block)} structured desktop canvas ${r.width.toFixed(0)}px < ${required.toFixed(0)}px at ${w}px`)}}
     if(document.body.dataset.archivePage==='writing'||document.body.dataset.archivePage==='community'){for(const section of document.querySelectorAll('main>section')){if(!visible(section)||!section.querySelector('.linklist'))continue;const r=section.getBoundingClientRect(),required=Math.min(w*.76,1400);if(r.width<required)out.push(`${name(section)} list section desktop canvas ${r.width.toFixed(0)}px < ${required.toFixed(0)}px at ${w}px`)}}
   }
