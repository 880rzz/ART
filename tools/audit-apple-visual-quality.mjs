...
for(const width of widths){
  ...
  for(const pathname of pages){
    ...
    const result=await page.evaluate(()=>{
      ...
      // Apple editorial readability: prose is left aligned, never justified, at a readable measure and leading.
      for(const el of longText){
        ...
        if(w<=768&&!el.closest('.gallery,.collage,.record-gallery,.archive-source-hub')){
          if(r.left<15)issues.push(`${name(el)} mobile/tablet left gutter ${r.left.toFixed(1)}px`);
          if(r.right>w-15)issues.push(`${name(el)} mobile/tablet right gutter ${(w-r.right).toFixed(1)}px`);
        }
        if(txt.length>500&&r.width<Math.min(280,w-40))issues.push(`${name(el)} reading column too narrow ${r.width.toFixed(0)}px`);
      }

      // Display hierarchy and line-height: clear Apple-like scale, no oversized or cramped display text.
      for(const h of document.querySelectorAll('main h1,main h2,main h3,header h1')){
        ...
        const lim=tag==='h1'?(w<=430?[32,50]:w<=768?[34,58]:[38,76]):tag==='h2'?(w<=430?[24,40]:[24,48]):[18,34];
        if(fs<lim[0]||fs>lim[1])issues.push(`${name(h)} font-size ${fs.toFixed(1)}px outside ${lim[0]}–${lim[1]}px`);
        if(w<=768&&!h.closest('.gallery,.collage')&&(r.left<15||r.right>w-15))issues.push(`${name(h)} heading violates 16px page gutter [${r.left.toFixed(1)},${r.right.toFixed(1)}]`);
      }

      // Curatorial dossier: the complaint-producing right-shift is forbidden. Headings, leads, metadata, lists and quotes share one reading axis.
      ...
      // Writing is editorial prose, therefore left aligned at every viewport.
      if(document.body.dataset.archivePage==='writing'){
        for(const el of document.querySelectorAll('main h1,main h2,main h3,main p,main li'))if(visible(el)&&getComputedStyle(el).textAlign!=='left')issues.push(`writing alignment ${name(el)}=${getComputedStyle(el).textAlign}`);
      }

      // Full-bleed cells/surfaces: visible section backgrounds belong to the viewport, content stays in the inner wrap.
      for(const sec of document.querySelectorAll('main > section')){
        if(!visible(sec))continue;const r=sec.getBoundingClientRect(),s=getComputedStyle(sec),before=getComputedStyle(sec,'::before');
        const bg=s.backgroundColor,bb=before.backgroundColor;
        const visibleOwn=bg!=='rgba(0, 0, 0, 0)'&&bg!==bodyBg;
        const visiblePseudo=bb&&bb!=='rgba(0, 0, 0, 0)'&&bb!==bodyBg;
        if(visibleOwn&&r.width<w-2)issues.push(`${name(sec)} colored cell is not full viewport width (${r.width.toFixed(0)}/${w})`);
        if(!visibleOwn&&visiblePseudo&&r.width<w-2){
          const pseudoWidth=px(before.width);const left=before.left,right=before.right;
          if(pseudoWidth<w-2&&left!=='0px'&&right!=='0px')issues.push(`${name(sec)} pseudo surface does not prove full-bleed width`);
        }
        if(s.contentVisibility==='auto')issues.push(`${name(sec)} content-visibility:auto can create blank visual bands`);
      }

      // Component spacing: headings should breathe but remain visually attached to their copy.
      for(const h of document.querySelectorAll('main h1,main h2,main h3')){
        if(!visible(h))continue;let n=h.nextElementSibling;while(n&&!visible(n))n=n.nextElementSibling;if(!n)continue;
        if(!n.matches('p,ul,ol,blockquote,.lead,.meta,.cards,.archive-grid,.press-facts'))continue;
        const a=h.getBoundingClientRect(),b=n.getBoundingClientRect();const gap=b.top-a.bottom;
      ...
      // Apple touch geometry on touch-sized layouts. Inline editorial links are excluded; controls are not.
      if(w<=768){
        const controls=[...document.querySelectorAll('button,summary,input:not([type="hidden"]),select,textarea,.btn,.menu-btn,.nav-links a')].filter(visible);
        for(const el of controls){const r=el.getBoundingClientRect();if(r.height<43.5)issues.push(`${name(el)} touch height ${r.height.toFixed(1)}px < 44px`);if((el.matches('button,.menu-btn')||el.getAttribute('role')==='button')&&r.width<43.5)issues.push(`${name(el)} touch width ${r.width.toFixed(1)}px < 44px`);}
      }

      // Card/cell integrity: meaningful cells need breathing room, no extreme radii or collapsed widths.
      for(const el of document.querySelectorAll('.card,.archive-card,.press-fact,.press-record,.curatorial-period,.t-item')){
        if(!visible(el))continue;const s=getComputedStyle(el),r=el.getBoundingClientRect();
        const pl=px(s.paddingLeft),pr=px(s.paddingRight),pt=px(s.paddingTop),pb=px(s.paddingBottom);
      ...
      stats.longText=longText.length;
      stats.sections=[...document.querySelectorAll('main>section')].filter(visible).length;
      stats.controls=[...document.querySelectorAll('button,summary,input:not([type="hidden"]),select,textarea,.btn,.menu-btn,.nav-links a')].filter(visible).length;
      return {issues:[...new Set(issues)].slice(0,160),stats};
    });
    reports.push({width,pathname,...result.stats,issues:result.issues.length});
...
fs.writeFileSync('artifacts/apple-visual-quality.json',JSON.stringify({pages:pages.length,widths,reports,failures},null,2));
...
console.log(`ART Apple visual quality audit passed: ${pages.length} pages × ${widths.length} viewports; typography, leading, alignment, reading measure, gutters, full-bleed cells, touch geometry and visual spacing verified.`);
