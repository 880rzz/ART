...
function targetFile(source,hrefPath){
  let clean=decodeURIComponent(hrefPath||'');
  if(!clean) return source;
  if(clean.startsWith('/')) clean=clean.slice(1);
  else clean=path.posix.normalize(path.posix.join(path.posix.dirname(path.relative(root,source).replaceAll('\\','/')),clean));
  if(clean.endsWith('/')) clean+='index.html';
  else if(!path.posix.extname(clean)) clean=fs.existsSync(path.join(root,clean+'.html'))?clean+'.html':path.posix.join(clean,'index.html');
...
for(const file of files){
  ...
  for(const m of html.matchAll(/href=["']([^"']*#[^"']*)["']/gi)){
    ...
    const [p,fragRaw]=href.split('#',2);const frag=decodeURIComponent(fragRaw||'');if(!frag)continue;
    const target=targetFile(file,p);const set=ids(target);checked++;
    if(!set) failures.push(`${path.relative(root,file)}: fragment target file missing for ${href}`);
    else if(!set.has(frag)) failures.push(`${path.relative(root,file)}: fragment #${frag} missing in ${path.relative(root,target)}`);
  }
}
