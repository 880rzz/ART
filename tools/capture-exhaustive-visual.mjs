...
fs.writeFileSync(path.join(outDir,'manifest.json'),JSON.stringify({repo:'ART',pages:pages.length,widths,records:manifest},null,2));
console.log(`Captured ${pages.length} ART content pages at ${widths.join('/')}px with concurrency ${concurrency}; every main/footer disclosure also has an expanded-state full-page capture.`);
