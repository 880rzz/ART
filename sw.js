...
const V='banhalmi-art-20260726-final-v1';
...
self.addEventListener('fetch',e=>{const r=e.request;if(r.method!=='GET')return;const url=new URL(r.url);if(url.origin!==location.origin)return;if(r.mode==='navigate'){e.respondWith((async()=>{try{const n=await repairedResponse(await fetch(r,{cache:'no-store'}));const c=await caches.open(V);c.put(r,n.clone());return n;}catch(err){const c=await caches.open(V);const hit=(await c.match(r))||(await c.match('/'));return hit?repairedResponse(hit):Response.error();}})());return;}e.respondWith((async()=>{const c=await caches.open(V);const hit=await c.match(r);if(hit)return hit;try{const n=await fetch(r);if(n&&n.status===200&&n.type==='basic')c.put(r,n.clone());return n;}catch(err){return hit||Response.error();}})());});